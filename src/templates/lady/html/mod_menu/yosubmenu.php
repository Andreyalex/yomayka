...
foreach ($groups as $key => $group) {
    ...
    echo '<ul class="group items-count-'.count($group).' '.$class.'" data-parent="'.$key.'">';
...
<script language="javascript">
    ...
    !function($){
        var container = $('.role-menu-top-sub .groups-container');
        var levelTop = $('.role-menu-top .nav');
        var interval = null;
        var groupsList = container.find('.group');

        groupsList.each(function(){
            var group = this;
            var parentId = $(group).attr('data-parent');
            var parent = levelTop.find('.item-'+parentId+' a');
            if (parent.length > 0) {
