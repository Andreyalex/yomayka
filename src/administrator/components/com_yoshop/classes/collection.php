...
class YoshopCollection implements ArrayAccess, Iterator, Countable
{
    ...
    protected function _getItem($idx)
    {
        ...
        if (!$item instanceof $this->_rowClass) {
            ...
            if ($this->_rowClass) {
                ...
                $this->_collection[$idx] = ($parts[0] == 'Yoshop')?
                    $this->di->createModel($parts[1], 'YoshopModel', array('state' => new JObject($item))) :
