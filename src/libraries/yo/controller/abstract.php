...
class YoControllerAbstract extends JControllerAdmin
{
    public $context;
    ...
    public function getSuffix()
    {
        if (empty($this->suffix))
        {
            ...
            if (!preg_match('/Controller(.*)/i', get_class($this), $r))
            {
                throw new Exception(JText::_('JLIB_APPLICATION_ERROR_CONTROLLER_GET_SUFFIX'), 500);
            }
            $this->suffix = strtolower($r[1]);
        }
    ...
    public function save($key = null, $urlVar = null)
    {
        ...
        switch ($task)
        {
            case 'apply':
                ...
                $recordId || ($recordId = $model->data->id);
                $this->holdEditId($context, $recordId);
                $app->setUserState($context . '.data', null);
    ...
    /**
     * Automatically gets the default name of a model
     *
     * @param string $name
     * @param string $prefix
     * @param array $config
