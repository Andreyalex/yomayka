...
class JshoppingControllerSearch extends JController{
    ...
    function result(){
        ...
        if ($date_to && checkMyDate($date_to)) {
            $adv_query .= " AND prod.product_date_added <= '".$db->getEscaped($date_to)."'";
        }
        if ($date_from && checkMyDate($date_from)) {
            $adv_query .= " AND prod.product_date_added >= '".$db->getEscaped($date_from)."'";
        }                
        
        $where_search = "";
        if ($search_type=="exact"){
            $word = addcslashes($db->getEscaped($search), "_%");
