...
class JshoppingControllerCheckout extends JController{
    ...
    function step6iframe(){
        ...
        ?><iframe width="<?php print $width?>" height="<?php print $height?>" frameborder="0" src="<?php print SEFLink('index.php?option=com_jshopping&controller=checkout&task=step6&wmiframe=1',0,1,$jshopConfig->use_ssl)?>"><?php
    }
    ...
    function _sendOrderEmail($order_id){
        ...
        JPluginHelper::importPlugin('jshoppingorder');
        $dispatcher =& JDispatcher::getInstance();
        $dispatcher->trigger('onBeforeSendEmailsOrder', array(&$order, &$listVendors) );
    ...
    function _showCheckoutNavigation($step) {
        ...
        $dispatcher->trigger('onBeforeDisplayCheckoutNovigator', array(&$view));
        $view->display();
    }
